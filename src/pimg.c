...
static void ft_wall_size(t_all *settings)
{
    ...
    settings->dr.ds = (-settings->dr.h / 2 + settings->config.res[1]/2);//check
    if (settings->dr.ds < 0)
        settings->dr.ds = 0;
    settings->dr.de = (settings->dr.h / 2 + settings->config.res[1] / 2); //check
...
static void ft_putimg(t_all *settings)
{
    while (settings->bm.i < settings->config.res[0])
    {
        ...
        settings->bm.camera = (2 * settings->bm.i) / (double)settings->config.res[0] - 1;
