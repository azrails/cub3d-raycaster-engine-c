...
{
    settings->ds->dt = 1.0 / (settings->d.px * settings->d.y - settings->d.py * settings->d.x);
    settings->ds->tx = settings->ds->dt * (settings->d.y * settings->ds->sx - settings->d.x * settings->ds->sy);
    settings->ds->ty = settings->ds->dt * (settings->d.px * settings->ds->sy - settings->d.py * settings->ds->sx);
    settings->ds->ssx = (int)((settings->config.res[0] / 2) * (1 + settings->ds->tx / settings->ds->ty));
    settings->ds->h = abs((int)(settings->config.res[1] / settings->ds->ty));
    settings->ds->dsy = ((settings->config.res[1] / 2) - settings->ds->h / 2);
...
{
    settings->ds->sx = settings->ds->tsp[settings->ds->i].x - (settings->position.x - 0.5);
    settings->ds->sy = settings->ds->tsp[settings->ds->i].y - (settings->position.y - 0.5); 
    ft_value(settings);
    while (settings->ds->j < settings->ds->dex)
    {
        settings->ds->tsx = (int)(settings->psp->sl * (settings->ds->j - (settings->ds->ssx - settings->ds->w / 2)) 
            * settings->psp->wd / settings->ds->w) / settings->psp->sl;
        if (settings->ds->ty > 0 && settings->ds->j > 0 && settings->ds->j < settings->config.res[0] && settings->ds->ty < settings->bm.psp[settings->ds->j])
        {
